...
void Bullet::fire(const Vector2f &pos) {
    ...
    *angleshot = atan2(mousePos.y - bullets[bulletCount].getPosition().y,
                       mousePos.x - bullets[bulletCount].getPosition().x);
...
void Bullet::enemyFire(const sf::Vector2f &pos, const sf::Vector2f &playerPos) {
    ...
    *angleshot = atan2(playerPos.y - bullets[bulletCount].getPosition().y,
                       playerPos.x - bullets[bulletCount].getPosition().x);
    bullets[bulletCount].setAngle(*angleshot, bullets[bulletCount]);
    *angleshot = atan2(playerPos.y - bullets[bulletCount].getPosition().y,
                       playerPos.x - bullets[bulletCount].getPosition().x);
...
void Bullet::_update(const double dt) {
    ...
    if (isMarkedDestroy && elapsedTime > 0.00001) {
        bullet = "bulletErased";
        elapsedTime = 0.f;
    }
    // If bullet is out of bounds. remove/return;
    if (getPosition().x < view.getCenter().x - 100 - view.getSize().x * 0.5 || getPosition().x > view.getCenter().x +
        100 + view.getSize().x * 0.5
        || getPosition().y < view.getCenter().y - 100 - view.getSize().y * 0.5 || getPosition().y > view.getCenter().y +
        100 + view.getSize().y * 0.5) {
        this->isVisible = false;
        return;
    } else {
        this->move(cos(this->angle) * 150.f * dt, 0);
        this->move(0, sin(this->angle) * 150.f * dt);
    }
    ...
    for (auto enemy: enemies) {
        ...
        if (this->isVisible == true) {
            if (getPosition().x > gameWidth || getPosition().x < 0 || getPosition().y > gameHeight || getPosition().y < 0) {
                isMarkedDestroy = true;
                return;
            }
            if (LevelSystem::getTileAt(getPosition()) == LevelSystem::WALL && isMarkedDestroy != true) {
                isMarkedDestroy = true;
                this->isVisible = false;
                setPosition(-10000, -10000);
            }
        }
        if (enemy->isAlive() && spriteBounds.intersects(boundingBox) && isMarkedDestroy != true) {
            this->isVisible = false;
            setPosition(-10000, -10000);
            if (elapsedTime > 15.0) {
